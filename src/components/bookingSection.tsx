...
import { homepageBookingSchema, HomepageBookingSchema } from "@/lib/validation"
...
export function BookingSection() {
    ...
    } = useForm<HomepageBookingSchema>({
    resolver: zodResolver(homepageBookingSchema),
     })

  const onSubmit = (data: HomepageBookingSchema) => {
    console.log("Submitted Data:", data)
  }
  return (
    <section id="booking" className="w-full py-16 md:py-32 bg-gradient-to-br from-blue-50 to-cyan-50">
                            ...
                            <CardContent className="relative space-y-6">
                                <div className="grid gap-4 sm:grid-cols-2">
                                    <div className="space-y-2">
                                        ...
                                        <Input id="name" placeholder="eg. Michael Barker" {...register("name")} />
                                        {errors.name && <p className="text-red-500 text-sm">{errors.name.message}</p>}
                                    </div>
                                    <div className="space-y-2">
                                        <Label htmlFor="email">Email Address</Label>
                                        <Input id="email" type="email" placeholder="eg. michael@example.com" {...register("email")} />
                                        {errors.email && <p className="text-red-500 text-sm">{errors.email.message}</p>}
                                    </div>
                                    <div className="space-y-2">
                                        <Label htmlFor="preferred-date">Preferred Date</Label>
                                        <Input id="preferred-date" type="date" {...register("preferredDate")} />
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input id="service-type" placeholder="E.g., Legionella Test" {...register("serviceType")} />
                                    {errors.serviceType && <p className="text-red-500 text-sm">{errors.serviceType.message}</p>}
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="property-address">Property Address</Label>
                                    <Input id="property-address" placeholder="Enter address" {...register("propertyAddress")} />
                                    {errors.propertyAddress && <p className="text-red-500 text-sm">{errors.propertyAddress.message}</p>}
                                </div>

                                <Button type="submit" className="w-full bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white py-3 rounded-lg">
                                    Book Appointment
                                </Button>

                                {/* Spinner overlay (shown only when submitting) */}
                                {isSubmitting && (
